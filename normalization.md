...
The table has potential anomalies still because e.g. if the `product_price` of "Apple" changes, we may have to update multiple rows. If we miss one, we get inconsistencies. There are other rows that may contain Apple too.
